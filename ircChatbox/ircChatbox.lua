...
if not component.isAvailable("chat_box") then
	io.stderr:write("This Version of OpenIRC needs a Chat Box to broadcast messages to chat")
	return
end
...
local function print(message, overwrite,notChatbox)
  if not notChatbox then
	component.chat_box.say(message)
  end
...
local result, reason = pcall(function()
  ...
  local function chatBoxRead(causedEvent, chatBoxID, username,message)
	print("[" .. (target or "?") .. "] " .. nick .. ": " .. "<"..username.."> "..message, true,true)
	sock:write("PRIVMSG " .. target .. " :<"..username.."> "..message .. "\r\n")
	sock:flush()
  end
  
  local chatReader = event.listen("chat_message",chatBoxRead)
